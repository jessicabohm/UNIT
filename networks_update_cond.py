...
class Encoder_VAE(nn.Module):
    def __init__(self, n_downsample, n_res, input_dim, dim, norm, activ, pad_type, num_conds=0):
        super(Encoder_VAE, self).__init__()
        self.num_conds = num_conds
        self.model_preflat = []
        self.model_postflat = []
        if two_d:
            self.model_preflat += [Conv2dBlock(input_dim, dim, 7, 1, 3, norm=norm, activation=activ, pad_type=pad_type)]
        else:
            self.model_preflat += [Conv3dBlock(input_dim, dim, 7, 1, 3, norm=norm, activation=activ, pad_type=pad_type)]

        # downsampling blocks
        for i in range(n_downsample):
            if two_d:
                self.model_preflat += [Conv2dBlock(dim, 2 * dim, 4, 2, 1, norm=norm, activation=activ, pad_type=pad_type)]
            else:
                self.model_preflat += [Conv3dBlock(dim, 2 * dim, 4, 2, 1, norm=norm, activation=activ, pad_type=pad_type)]
            dim *= 2

        # residual blocks
        self.model_preflat += [ResBlocks(n_res, dim, norm=norm, activation=activ, pad_type=pad_type)]

        # NOTE: extra to map down to lower dim
        self.flatten = nn.Flatten()

        self.model_postflat += [
            nn.Linear(flattened_dim + num_conds, latent_dim),
        ...
        self.model_preflat = nn.Sequential(*self.model_preflat)
        self.model_postflat = nn.Sequential(*self.model_postflat)

    def forward(self, x, conds):
        x_conv = self.model_preflat(x)
        flatten = self.flatten(x_conv)
        cond_flatten = torch.cat((flatten, conds), dim=1) # concat the flatten latent vect to the conds for each batch
        out = self.model_postflat(cond_flatten)
...
class Decoder_VAE(nn.Module):
    def __init__(self, n_upsample, n_res, dim, output_dim, res_norm='in', activ='relu', pad_type='zero', num_conds=0): # NOTE: updated normalization to in to not have to compute weight and bias externally
        super(Decoder_VAE, self).__init__()

        self.model = []

        self.model += [
            nn.Linear(latent_dim + num_conds, flattened_dim),
...
class Segmentation3DDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = sitk.GetArrayFromImage(image).astype(np.float32)  # [D, H, W]
