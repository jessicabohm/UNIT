...
from networks_update_cond import *
import csv
import time
...
train_save_folder = "./VAE_train/3d_anat/mouse_train_cond_common/"
...
def loss_func(imgs, recons, means, log_vars):
    weights = torch.ones(271, device=recons.device)
    weights[251:] = 10.0
...
common_vols = np.arange(20)
train_dataset = Segmentation3DDataset(train_folder, common_vols=common_vols, return_vols=True)
train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)

val_dataset = Segmentation3DDataset(val_folder, common_vols=common_vols, return_vols=True)
val_loader = DataLoader(val_dataset, batch_size=1, shuffle=False)

# Initialize model
encoder = Encoder_VAE(n_downsample=n_downsample, n_res=1, input_dim=1, dim=dim, norm='in', activ='relu', pad_type='zero', num_conds=len(common_vols)) # encodes to 32 dim??
decoder = Decoder_VAE(n_upsample=n_downsample, n_res=1, dim=encoder.output_dim, output_dim=271, num_conds=len(common_vols))
...
for i, epoch in enumerate(range(start_epoch, start_epoch + num_epochs + 1)):
    ...
    for batch_idx, (batch, conds) in enumerate(train_loader):
        batch = batch.to(device)  # (B, C, D, H, W)
        conds = conds.to(device)

        # Forward pass
        means, log_vars = encoder(batch, conds)

        # get latent vectors - sampled from learned dists
        z = reparameterization(means, log_vars)
        z_cond = torch.cat((z, conds), dim=1)
    ...
    with torch.no_grad():
        for i, (val_batch, conds) in enumerate(val_loader):
            val_batch = val_batch.to(device)  # (B, C, D, H, W)
            conds = conds.to(device)

            # Forward pass
            means, log_vars = encoder(val_batch, conds)
