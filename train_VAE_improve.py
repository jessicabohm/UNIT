...
decoder = Decoder_VAE(n_upsample=n_downsample, n_res=1, dim=encoder.output_dim, output_dim=271)
...
# ===============================
def loss_func(imgs, recons, means, log_vars, beta):
    # Reconstruction loss
    criterion = nn.CrossEntropyLoss()
    recon = criterion(recons, imgs)  # imgs: [B,D,H,W], recons: [B,C,D,H,W]

    # Variance clamping to prevent collapse
    log_vars = torch.clamp(log_vars, min=-10.0, max=10.0)  
    std = torch.exp(0.5 * log_vars)

    BS = imgs.size(0)
    num_voxels = img_x * img_y * img_z
    KLD = (-0.5 * torch.sum(1 + log_vars - means.pow(2) - std.pow(2))) / (num_voxels * BS)
...
def reparameterization(means, log_vars):
    ...
    std = torch.exp(0.5 * torch.clamp(log_vars, min=-10.0, max=10.0))
    z = means + std * epsilon
    return z
...
# ===============================
def kl_anneal_function(epoch, max_beta=1.0, anneal_epochs=50):
    """ Linearly increase beta over `anneal_epochs` """
...
for i, epoch in enumerate(range(start_epoch, start_epoch + num_epochs + 1)):
    ...
    # Update beta via KL annealing
    beta = kl_anneal_function(epoch, max_beta=1.0, anneal_epochs=50)
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        # Compute loss
        recon_loss, KLD_loss = loss_func(batch.squeeze(1).long(), recon, means, log_vars, beta)
        loss = (recon_loss + KLD_loss) / accum_steps
    ...
    # Final step if not divisible by accum_steps
    if (batch_idx + 1) % accum_steps != 0:
        optimizer.step()
        optimizer.zero_grad()

    # Validation loop (unchanged, but now using annealed beta)
    encoder.eval()
    decoder.eval()
    val_loss, val_recon_loss, val_KLD_loss = 0, 0, 0

    img_to_save, recon_to_save = [], []
    img_vol_ratios, recon_vol_ratios = [], []
    ...
    with torch.no_grad():
        for i, val_batch in enumerate(val_loader):
            ...
            recon_loss, KLD_loss = loss_func(val_batch.squeeze(1).long(), recon, means, log_vars, beta)
            loss = (recon_loss + KLD_loss) / accum_steps
